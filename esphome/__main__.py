...
def generate_cpp_contents(config: ConfigType) -> None:
    ...
    for name, manifest, conf in entries:
        _LOGGER.error("Processing component %s", name)
    ...
    for name, manifest, _comp_conf in entries:
        ...
        for dep in manifest.dependencies:
            if dep not in name_to_comp_conf:
                ...
                raise EsphomeError(
                    f"Dependency {dep} not found in {name_to_comp_conf}. This is likely a bug"
                )
            add_edge(dep, name)
        ...
        for dep in auto_load or []:
            add_edge(name, dep)

        if manifest.is_target_platform:
            # target platforms do not depend on anything.
            continue

        if name == CONF_ESPHOME:
            add_edge(str(CORE.target_platform), CONF_ESPHOME)
            continue
    ...
    _LOGGER.error("Adjacency list: %s", adj)
    ...
    _LOGGER.error("Partial topo order: %s", partial_order)
    _LOGGER.error("Total topo-names order: %s", ordered_names)
